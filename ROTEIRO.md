...
Guia prático para rodar a análise no macOS, tanto para grupos focais quanto
para outros corpora qualitativos (entrevistas, respostas abertas, documentos).
Todos os comandos partem da raiz do repositório. O Python do ambiente é
`./.venv/bin/python` (abreviado abaixo como `PY`). Defina antes, uma vez por sessão:
...
Ver `SETUP_DICIONARIOS.md` para obter os dicionários do IRaMuTeQ e, para a
distância de Labbé, `LABIIALEX_RSCRIPTS`.
...
arquivo, útil para comparar documentos. O repositório traz o corpus sintético
`corpus_synth` para teste.
...
```
$PY run_prepare.py --corpus meu_corpus --out output/ESTUDO \
    --stopwords config_med/stopwords.txt
```
...
```
$PY -c "import pandas as pd; d=pd.read_csv('output/ESTUDO/forms.csv',sep=';'); print(d.sort_values('frequencia',ascending=False).head(30).to_string(index=False))"
```

Acrescente à lista de stopwords (`config_med/stopwords.txt`, sempre em texto
puro, edite no VS Code) o que for ruído no seu contexto: marcações de
...
```
$PY run_all.py --corpus meu_corpus --out output/ESTUDO \
    --stopwords config_med/stopwords.txt \
...
gera `output/ESTUDO/report.html`, um relatório único com as figuras.

### Opção B: passo a passo (para interpretar com calma)

```
$PY run_chd.py       --prepared output/ESTUDO --n-classes 4 --stability
$PY run_afc_spec.py  --prepared output/ESTUDO
$PY run_simi.py      --prepared output/ESTUDO --top 80 --layout fa2
$PY run_lda.py       --prepared output/ESTUDO --k 4 --level uce --stability
$PY run_emotions.py  --prepared output/ESTUDO
$PY run_kwic.py      --prepared output/ESTUDO --query PALAVRA
$PY run_extras.py    --prepared output/ESTUDO --what wordcloud
$PY run_extras.py    --prepared output/ESTUDO --what ngrams --n 2
$PY run_extras.py    --prepared output/ESTUDO --what wordtree --query PALAVRA
$PY run_extras.py    --prepared output/ESTUDO --what heatmap
$PY run_extras.py    --prepared output/ESTUDO --what yake --n 3
$PY report.py        --prepared output/ESTUDO
```
...
```
for k in 2 3 4 5; do $PY run_chd.py --prepared output/ESTUDO --n-classes $k --stability 2>&1 | grep -E "classes:|Rand|Estabilidade"; done
```
